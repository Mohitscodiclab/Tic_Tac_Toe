...
namesDialogButton.addEventListener('click', (event) => {
    ...
    const player1Name = form.querySelector('#name1').value.trim();
    const player2Name = form.querySelector('#name2').value.trim();

    // Check if either name is empty
    if (player1Name === '' || player2Name === '') {
        alert("⚠️ B*h*n ke L**d, fill all the fields before starting the game.");
        return;
    }

    // Both names are filled – proceed to game
